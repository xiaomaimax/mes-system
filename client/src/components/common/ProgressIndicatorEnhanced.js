/**
 * ProgressIndicatorEnhanced - 增强的进度指示器组件
 * 
 * 功能：
 * - 显示数据保存、加载、同步等操作的进度
 * - 支持多种进度显示模式
...
import { Progress, Spin, Alert, Button, Space, Typography, Card, Tooltip, Collapse, Empty, Divider, Tag } from 'antd';
import { 
  LoadingOutlined, 
  CheckCircleOutlined, 
...
/**
 * 增强的浮动进度提示组件 - 支持多个并发操作
 */
export const FloatingProgressEnhanced = ({
  visible = false,
  operation = 'save',
  progress = 0,
  status = PROGRESS_STATUS.IDLE,
      ...
      <Card 
        ...
        title={
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span><HistoryOutlined /> 操作历史</span>
            <Space size="small">
              <Tooltip title="撤销">
                <Button 
                  size="small" 
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>
                  ...
                  <Tag color={op.status === 'success' ? 'green' : 'red'} style={{ marginRight: '4px' }}>
                    {op.type}
                  </Tag>
                  {op.message}
                </span>
  ...
  const renderPerformanceMetrics = () => {
    ...
    const { averageOperationTime, operationCount, fastestOperation, slowestOperation, slowOperations } = performanceMetrics;
    ...
    return (
      <Card 
        ...
        title={<span>性能指标</span>}
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Text>总操作数:</Text>
            <Text strong>{operationCount}</Text>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Text>平均耗时:</Text>
            <Text strong>{averageOperationTime}ms</Text>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Text>最快:</Text>
            <Text strong>{fastestOperation}ms</Text>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Text>最慢:</Text>
            <Text strong>{slowestOperation}ms</Text>
          </div>
          
          {slowOperations && slowOperations.length > 0 && (
            <>
              <Divider style={{ margin: '8px 0' }} />
              <Text type="warning">慢操作 ({slowOperations.length})</Text>
              <div style={{ maxHeight: '100px', overflowY: 'auto', fontSize: '12px' }}>
                {slowOperations.map((op, index) => (
                  <div key={index} style={{ padding: '4px 0' }}>
                    <Text type="secondary">{op.time}ms - {new Date(op.timestamp).toLocaleTimeString()}</Text>
                  </div>
                ))}
              </div>
            </>
