/**
 * useUIFeedbackEnhanced - 增强的UI反馈状态管理Hook
 * 
 * 功能：
 * - 管理加载、进度、错误等UI状态
 * - 提供统一的UI反馈接口
 ...
 * 增强的UI反馈Hook
 */
const useUIFeedbackEnhanced = (options = {}) => {
  const {
  ...
  // 并发操作管理
  const [concurrentOperations, setConcurrentOperations] = useState(new Map());
  ...
  const updatePerformanceMetrics = useCallback((operationTime) => {
    ...
    setPerformanceMetrics(prev => {
      ...
      const newFastest = Math.min(prev.fastestOperation, operationTime);
      const newSlowest = Math.max(prev.slowestOperation, operationTime);
      
      // 记录慢操作（超过200ms）
      const newSlowOps = operationTime > 200 
        ? [...prev.slowOperations, { time: operationTime, timestamp: new Date().toISOString() }].slice(-10)
        : prev.slowOperations;
      ...
      return {
        ...
        slowOperations: newSlowOps,
        fastestOperation: newFastest,
        slowestOperation: newSlowest
      };
    });
  }, [trackPerformance]);
  
  // 添加并发操作
  const addConcurrentOperation = useCallback((operationId, operationData) => {
    setConcurrentOperations(prev => {
  ...
  // 移除并发操作
  const removeConcurrentOperation = useCallback((operationId) => {
    setConcurrentOperations(prev => {
      const newMap = new Map(prev);
  ...
  const undo = useCallback(() => {
    ...
    setHistoryIndex(prev => prev - 1);
    const previousOp = operationHistory[historyIndex - 1];
    
    if (previousOp) {
      setMessage(`已撤销: ${previousOp.message}`);
      safeMessage.info(`已撤销: ${previousOp.message}`);
    }
  }, [enableUndo, historyIndex, operationHistory]);
  
  // 重做操作
  const redo = useCallback(() => {
    if (!enableRedo || historyIndex >= operationHistory.length - 1) return;
    
    setHistoryIndex(prev => prev + 1);
    const nextOp = operationHistory[historyIndex + 1];
    
    if (nextOp) {
      setMessage(`已重做: ${nextOp.message}`);
      safeMessage.info(`已重做: ${nextOp.message}`);
    }
  }, [enableRedo, historyIndex, operationHistory]);
  ...
  // 获取并发操作
  const getConcurrentOperations = useCallback(() => {
    return Array.from(concurrentOperations.values());
  ...
  return {
    ...
    canRedo: enableRedo && historyIndex < operationHistory.length - 1,
...
export default useUIFeedbackEnhanced;
